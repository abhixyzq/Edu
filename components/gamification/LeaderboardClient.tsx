...
export function LeaderboardClient() {
  ...
  const [dbUsers, setDbUsers] = useState<LeaderboardUser[]>([]);
  ...
  // ─── Fetch Real Registered Users from Supabase ───
  useEffect(() => {
    let isMounted = true;
    ...
    async function fetchRealLeaderboard() {
      ...
      try {
        ...
        if (error) {
          ...
          const mapped: LeaderboardUser[] = data.map((p: any, idx: number) => {
            ...
            // Calculate accuracy from completed_nodes if available
            const completed = p.completed_nodes || {};
            const completedList = Object.values(completed) as Array<{ score?: number; stars?: number }>;
            const testsCount = completedList.length;
            const avgAcc =
              testsCount > 0
                ? Math.round(
                    completedList.reduce((sum, n) => sum + (Number(n.score) || 85), 0) / testsCount
                  )
                : (Number(p.xp_points) > 0 ? 88 : 0);
            ...
            return {
              ...
              board: boardLabel,
              boardKey: bKey,
              specialization: p.class_level ? `${p.class_level} Prep` : 'Board Prep',
              xp: Number(p.xp_points) || 0,
              accuracy: avgAcc,
              ...
              isCurrentUser: Boolean(
                ...
                (user.username && rawUser === user.username)
              ),
            };
          });
        ...
        console.warn('[Leaderboard] Failed to load real users:', err);
      } finally {
        if (isMounted) setLoading(false);
      }
    }
  ...
  }, [user.id, user.username, user.xp]);

  // ─── Assemble Combined Real Roster (Supabase profiles + Current local session if unlisted) ───
  const rawRoster: LeaderboardUser[] = useMemo(() => {
    let list = [...dbUsers];

    // Check if current user is present
    const hasCurrent = list.some((u) => u.isCurrentUser || (user.id && u.id === user.id));

    if (hasCurrent) {
      // Update the current user entry with latest local UserContext values
      list = list.map((u) => {
        if (u.isCurrentUser || (user.id && u.id === user.id)) {
          return {
            ...u,
    ...
    } else if (user.isLoggedIn || user.name) {
      // Current user exists locally but not in DB list yet (e.g. fresh signup or offline fallback)
      const rawName = user.name || 'You';
      const rawUser = user.username || 'scholar_12';
      const initials = rawName
        .split(' ')
        .map((w: string) => w[0])
        .join('')
      ...
      const bRaw = (user.targetBoard || 'cbse').toLowerCase();
      let bKey = 'cbse';
      let bLabel = `CBSE ${user.classLevel || 'Class 12'}`;
      if (bRaw.includes('bseb') || bRaw.includes('bihar')) {
        bKey = 'bseb';
        bLabel = 'Bihar Board (BSEB)';
      } else if (bRaw.includes('up')) {
        bKey = 'up';
        bLabel = 'UP Board';
      } else if (bRaw.includes('icse') || bRaw.includes('isc')) {
        bKey = 'icse';
        bLabel = 'ICSE / ISC';
      }
      ...
      list.push({
        ...
        board: bLabel,
        boardKey: bKey,
        specialization: `${user.classLevel || 'Class 12'} Prep`,
        xp: user.xp,
        accuracy: user.xp > 0 ? 90 : 0,
        streak: user.streakDays || 0,
        testsCompleted: Object.keys(user.completedNodes || {}).length,
        trend: 'neutral',
        trendValue: 0,
        isCurrentUser: true,
      });
  ...
  }, [dbUsers, user.id, user.name, user.username, user.xp, user.streakDays, user.targetBoard, user.classLevel, user.avatarUrl, user.isLoggedIn, user.completedNodes]);
  ...
  const filteredList = useMemo(() => {
    ...
    if (selectedBoard !== 'all') {
      list = list.filter((u) => u.boardKey === selectedBoard);
    }

    // Re-index ranks based on the filtered view
    return list.map((u, i) => ({ ...u, rank: i + 1 }));
  ...
  const currentUserData = useMemo(() => {
    return filteredList.find((u) => u.isCurrentUser) || rawRoster.find((u) => u.isCurrentUser) || filteredList[0] || {
      id: 'current-user',
      rank: 1,
      name: user.name || 'You',
  ...
  return (
    <main className="w-full min-h-screen bg-[#f4f5fa] pb-32 font-sans select-none overflow-x-hidden">
      ...
      <div className="max-w-md mx-auto px-3.5 sm:px-6 space-y-2.5 mt-1">
        ...
        <div className="bg-white rounded-3xl p-3 sm:p-3.5 border-2 border-b-4 border-[#e2e8f0] shadow-sm relative overflow-hidden flex items-center justify-between gap-3">
          <div className="min-w-0 flex-1">
            ...
            <h3 className="font-heading text-xs sm:text-sm font-black text-slate-900 leading-tight">
              Real Scholar Rankings 🟢
            </h3>
            <p className="text-[10px] text-slate-500 mt-0.5 leading-snug">
              {filteredList.length <= 3 ? (
                <span className="text-emerald-700 font-bold">
                  {currentUserData.rank === 1 ? '🥇 You are leading the ranking!' : `You are Rank #${currentUserData.rank}! Complete more tests to stay on top.`}
                </span>
        ...
        {loading && (
          <div className="py-6 flex flex-col items-center justify-center gap-2 bg-white rounded-3xl border-2 border-[#e2e8f0]">
          ...
          <div className="bg-white rounded-3xl p-3 border-2 border-b-4 border-[#e2e8f0] shadow-sm">
            <div className="text-center mb-1">
              <span className="text-[9px] font-black uppercase tracking-wider text-slate-400">
                {filteredList.length >= 3 ? 'Top 3 Podium' : filteredList.length === 2 ? 'Top 2 Podium' : 'Current Leader'}
              </span>
            </div>

            {filteredList.length === 1 ? (
              /* Single Leader Showcase */
              <div className="flex flex-col items-center justify-center py-2 text-center">
                ...
                >
                  <div className="relative mb-1">
                    <div className="w-16 h-16 rounded-2xl bg-gradient-to-tr from-amber-400 via-yellow-300 to-amber-500 border-3 border-amber-300 p-0.5 shadow-md shadow-amber-200 flex items-center justify-center overflow-hidden">
                      {top1.avatarUrl || (top1.isCurrentUser && user.avatarUrl) ? (
                        ...
                        <span className="text-base font-black text-amber-950">{top1.avatar}</span>
                      )}
                    </div>
                    <span className="absolute -bottom-1 -right-1 w-6 h-6 rounded-full bg-amber-400 border-2 border-white text-amber-950 font-black text-xs flex items-center justify-center shadow-xs">
                      🥇
                    </span>
                  </div>

                  <p className="text-xs font-black text-amber-950 truncate max-w-[140px] leading-tight">
                    {top1.name}
                  </p>
                  <p className="text-[10px] font-bold text-amber-700 truncate max-w-[140px]">
                    @{top1.username}
                  </p>

                  <div className="mt-1.5 py-0.5 px-3 rounded-xl bg-gradient-to-b from-amber-100 to-amber-200 border border-amber-300 flex items-center justify-center shadow-2xs">
                    <span className="text-xs font-black text-amber-950">{top1.xp} <span className="text-[9px] text-amber-800 font-normal">XP</span></span>
                  </div>
                </div>

                <p className="text-[10px] font-bold text-slate-400 mt-3">
                  🔥 Be the next challenger to climb the podium!
                </p>
              </div>
            ) : (
              /* Multi-Podium Grid (2 or 3 users) */
              <div className="grid grid-cols-3 gap-1.5 items-end pt-1 pb-0.5">
                ...
                {top2 ? (
                  ...
                  <div className="flex flex-col items-center opacity-40 text-center">
                    <div className="w-12 h-12 rounded-2xl border-2 border-dashed border-slate-300 flex items-center justify-center text-slate-300 text-lg mb-1">
                      🥈
                    </div>
                    <span className="text-[9px] font-bold text-slate-400">Open Slot</span>
                  </div>
                )}
        ...
        {!loading && filteredList.length === 0 && (
          <div className="bg-white rounded-3xl p-6 border-2 border-[#e2e8f0] text-center space-y-2">
            <span className="text-3xl">🎯</span>
            <h4 className="font-heading font-black text-sm text-slate-900">No Scholars on this Board Yet</h4>
            <p className="text-xs text-slate-500 max-w-xs mx-auto">
              Be the first scholar from this board to take a test and claim the #1 spot on the leaderboard!
            </p>
            <Link
              href="/tests"
              onClick={playButtonClick}
              className="inline-flex items-center gap-1.5 px-4 py-2 mt-2 rounded-2xl bg-[#7c3aed] text-white font-bold text-xs shadow-md shadow-purple-500/20 active:scale-95 transition-all"
            >
          ...
          <div className="space-y-1.5">
            <div className="flex items-center justify-between px-1">
              <span className="text-[11px] font-black text-slate-500 uppercase tracking-wider">
                {filteredList.length} {filteredList.length === 1 ? 'Scholar' : 'Scholars'} • {currentLeague.emoji} {currentLeague.name}
              </span>
            ...
            {remainingPlayers.map((player) => {
              ...
              const isDemotion = filteredList.length >= 13 && player.rank >= filteredList.length - 2;
              ...
              return (
                <React.Fragment key={player.id}>
                  ...
                  {player.rank === 7 && filteredList.length > 7 && (
                  ...
                  {/* 🔴 Demotion Zone Divider Line before Bottom 3 (only if >= 13 players) */}
                  {filteredList.length >= 13 && player.rank === filteredList.length - 3 && (
          ...
          <div className="flex items-center gap-2 min-w-0">
            <div className={`w-8 h-8 rounded-xl border flex items-center justify-center font-black text-[11px] overflow-hidden shrink-0 ${
              currentUserData.rank <= 3
                ...
                : filteredList.length >= 13 && currentUserData.rank >= filteredList.length - 2
            ...
            <div className="min-w-0">
              ...
              <p className="text-[10px] truncate mt-0.5">
                {currentUserData.rank === 1 ? (
                  <span className="text-amber-700 font-bold">🥇 Top of the leaderboard! ({user.xp} XP)</span>
                ) : currentUserData.rank <= 7 ? (
            ...
            <div className="flex items-start justify-between pb-2.5 border-b border-slate-100">
              ...
              <span className={`text-xs font-black px-2 py-0.5 rounded-full border ${
                selectedUser.rank <= 7
                  ...
                  : filteredList.length >= 13 && selectedUser.rank >= filteredList.length - 2
