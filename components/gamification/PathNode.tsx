...
export const PathNode: React.FC<PathNodeProps> = ({
  ...
  return (
    <div className="relative flex items-center justify-center select-none">
      ...
      {/* ─── Circular Level Node (Dead Center on Road Path) ─── */}
      <button
        type="button"
        onClick={handleClick}
        aria-label={`Stage ${levelNumber} - ${title}`}
        className={`relative w-[78px] h-[78px] sm:w-[84px] sm:h-[84px] rounded-full p-1 transition-all duration-200 flex items-center justify-center bg-white cursor-pointer hover:scale-105 active:scale-95 shrink-0 ${
          isActive
      ...
      {/* ─── Side Content Information (Positioned Safely to Side without Road Overlap) ─── */}
      <div
        className={`absolute top-1/2 -translate-y-1/2 flex flex-col pointer-events-none w-[160px] sm:w-[180px] ${
          textSide === 'left'
      ...
      >
        {/* Title */}
        <h3 className="font-heading text-sm sm:text-base font-black text-slate-800 leading-tight mb-1.5 pointer-events-auto">
          {title}
        </h3>
        ...
        <div className="pointer-events-auto">
          {isActive ? (
            <button
              type="button"
              onClick={handleClick}
              className="px-3.5 py-1 rounded-full bg-[#ff6937] text-white font-black text-[9px] sm:text-[10px] tracking-widest uppercase shadow-sm flex items-center gap-1 hover:bg-[#e85a2b] transition-colors cursor-pointer animate-pulse"
            >
            ...
            <button
              type="button"
              onClick={handleClick}
              className="px-3.5 py-1 rounded-full bg-[#22c55e] text-white font-black text-[9px] sm:text-[10px] tracking-widest uppercase shadow-xs flex items-center gap-1 hover:bg-[#16a34a] transition-colors cursor-pointer"
            >
              <span>COMPLETED</span>
            </button>
          ) : (
